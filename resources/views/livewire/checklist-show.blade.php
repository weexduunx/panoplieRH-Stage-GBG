                                ...
                                <tr>
                                        ...
                                        @if (!is_null($list_type))
                                            <div style="font-style: italic; font-size: 11px">
                                                ...
                                                @if (optional($user_tasks->where('tache_id', $tache->id)->first())->due_date)
                                                    | {{ __('Echéance') }}
                                                    {{ $user_tasks->where('tache_id', $tache->id)->first()->due_date->format('M d, Y') }}
                                                @endif
                                            </div>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        @if (optional($user_tasks->where('tache_id', $tache->id)->first())->is_important)
                                            <a wire:click.prevent="mark_as_important({{ $tache->id }})"
                                                class="bx bxs-star" href="#">

                                            </a>
                                        @else
                                            <a wire:click.prevent="mark_as_important({{ $tache->id }})"
                                                class="bx bx-star" href="#">
                ...
                <div class="card-header d-flex align-items-center justify-content-between">
                    ...
                    <div class="">
                        ...
                        @if ($current_task->is_important)
                            <a wire:click.prevent="mark_as_important({{ $current_task->id }})"
                                class="bx bxs-star"href></a>
                        @else
                            <a wire:click.prevent="mark_as_important({{ $current_task->id }})"
                                class="bx bx-star"href></a>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @if ($current_task->reminder_at)
                        {{ __('Rappel effectué:') }} {{ $current_task->reminder_at }}
                        &nbsp;&nbsp;
                        <a href="#" wire:click.prevent="set_reminder({{ $current_task->id }})">{{ __('Supprimer') }}</a>
                        @else
                        <a href="#" wire:click.prevent="toggle_reminder">{{ __('Rappel Moi') }}</a>
                        @if ($reminder_opened)
                        <ul>
                            <li>
                                <a wire:click.prevent="set_reminder({{ $current_task->id }}, '{{ today()->addDay()->toDateString() }}')"
                                   href="#">{{ __('Demain') }} {{ date('H') }}:00</a>
                            </li>
                            <li>
                                <a wire:click.prevent="set_reminder({{ $current_task->id }}, '{{ today()->addWeek()->startOfWeek()->toDateString() }}')"
                                   href="#">{{ __('Lundi Prochain') }} {{ date('H') }}:00</a>
                            </li>
                            <li>
                                {{ __('Ou choisissez une Date et une heure') }}
                                {{-- <div class="row">
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-md-3">
                                        <button wire:click.prevent="set_reminder({{ $current_task->id }}, 'custom')"
                                                class="btn btn-sm btn-primary">{{ __('Enregistrer') }}</button>
                                    </div>
                                </div> --}}

                                <div class="input-group">
                                    <input type="date"  wire:model="reminder_date" class="form-control" >
                                    <select wire:model="reminder_hour" class="form-control">
                                        @foreach (range(0,23) as $hour)
                                            <option value="{{ $hour }}">{{ $hour }}:00</option>
                                        @endforeach
                                    </select>
                                    <button class="btn btn-outline-primary" wire:click.prevent="set_reminder({{ $current_task->id }}, 'custom')" type="button" id="inputGroupFileAddon04">{{ __('Enregistrer') }}</button>
                                  </div>
                            </li>
                        </ul>
                        @endif
                    ...
                    @if ($current_task->due_date)
                        ...
                        &nbsp;&nbsp;
                        <a href="#"
                            wire:click.prevent="set_due_date({{ $current_task->id }})">{{ __('Supprimer') }}</a>
                    @else
                        <a href="#"
                            wire:click.prevent="toggle_due_date">{{ __('Ajouter une date d\'échéance') }}</a>
                        @if ($due_date_opened)
                            <ul class="timeline">
                                ...
                                <li class="timeline-item timeline-item-transparent">
                                    ...
                                    <div class="timeline-event">
                                        ...
                                        <div class="d-flex">
                                            <input wire:model="due_date" class="form-control" type="date"
                                                id="html5-date-input" name="picker_date">
                                        </div>
                                    </div>
                                </li>
                            </ul>
                ...
                <div class="card-body">
                    ...
                    @if ($note_opened)
                        ...
                        <div class="justify-content-end">
                            <div class="col-sm-10">
                                <button type="submit" wire:click="save_note"
                                    class="btn btn-primary">{{ __('Sauvegarder') }}</button>
                            </div>
                        </div>
                    @endif
                </div>
